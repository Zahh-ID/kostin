<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <a href="{{ route('owner.shared-tasks.index') }}" class="text-sm text-gray-600">&larr; {{ __('Cancel') }}</a>
            <h1 class="text-2xl font-semibold mt-2 mb-4">{{ __('Create Shared Task') }}</h1>
                ...
                <form>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="{{ __('e.g., Clean common area, Check fire extinguisher') }}">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700">{{ __('Description') }}</label>
                            <textarea class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" rows="3" placeholder="{{ __('Provide a detailed description of the task.') }}"></textarea>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                @foreach ($assignees as $assignee)
                                    <option value="{{ $assignee->id }}">{{ $assignee->name }} ({{ $assignee->role }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
