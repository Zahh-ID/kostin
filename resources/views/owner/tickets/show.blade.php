<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <small class="text-muted">{{ __('Tindak lanjut laporan penyewa dan perbarui statusnya.') }}</small>
        </div>
        <a href="{{ route('owner.tickets.index') }}" class="btn btn-outline-secondary btn-sm">
            {{ __('Kembali') }}
        </a>
    </x-slot>
            ...
            <div class="col-lg-5 col-xl-4">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-header bg-white border-0">
                        <h3 class="h6 mb-0">{{ __('Perbarui Status') }}</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('owner.tickets.update', $ticket) }}" method="post">
                            @csrf
                            @method('patch')
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="comment" class="form-label">{{ __('Catatan / Tindak lanjut') }}</label>
                                <textarea name="comment" id="comment" rows="4" class="form-control @error('comment') is-invalid @enderror" placeholder="{{ __('Berikan update untuk penyewa...') }}">{{ old('comment') }}</textarea>
                                @error('comment')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text">{{ __('Catatan akan dikirim sebagai komentar tiket.') }}</div>
                            </div>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white border-0">
                        <h3 class="h6 mb-0">{{ __('Timeline Status') }}</h3>
                    </div>
                    <div class="card-body">
                        ...
                        @empty
                            <p class="text-muted small mb-0">{{ __('Belum ada timeline status tersedia.') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
            <div class="col-lg-7 col-xl-8">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center">
                        <h3 class="h6 mb-0">{{ __('Diskusi dengan Penyewa') }}</h3>
                    ...
                    <div class="card-body">
                        ...
                        @empty
                            <p class="text-muted mb-0">{{ __('Belum ada komentar tambahan.') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
